...
>
    ...
    @else
        <button
            {{ $attributes->except('class') }} class="flex items-center space-x-1 text-left text-sm leading-4 font-medium">
            <span>{{ $slot }}</span>
            ...
            <span>
                ...
                @else
                    <svg
                        class="opacity-0 group-hover:opacity-100 transition-opacity duration-300 w-3 h-3 text-gray-800 dark:text-white"
                        aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
